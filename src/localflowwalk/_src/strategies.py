"""Neighbor query strategies for the walk.

Provides instance-based strategies used by ``walk_local_flow`` to find nearby
neighbors:
- ``BruteForceStrategy()``: Compute distances to all remaining points (default)
- ``KDTreeStrategy(k=...)``: Use a KD-tree for spatial prefiltering
    (requires optional ``jaxkd`` dependency)

Pass a strategy instance via the ``strategy`` parameter to ``walk_local_flow``.
"""
...
class QueryResult(NamedTuple):
    ...
    indices : Array
        Candidate indices (for kdtree, indices of k nearest neighbors)
        For brute-force, this is None or full index range.
...
class BruteForceStrategy(AbstractQueryStrategy):
...
class KDTreeStrategy(AbstractQueryStrategy):
    ...
    def __init__(self, k: int = 50) -> None:
        ...
        try:
            ...
            msg = (
                "KDTreeStrategy requires jaxkd optional dependency. "
