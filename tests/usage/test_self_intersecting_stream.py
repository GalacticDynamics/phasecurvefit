...
class TestSelfIntersectingStream:
    ...
    def test_walk_with_momentum_preserves_ordering(self):
        ...
        back_no = count_backward_steps(t, res_no_mom.ordered_indices)
        back_yes = count_backward_steps(t, res_with_mom.ordered_indices)
    ...
    def test_autoencoder_learns_gamma_from_true_ordering(self):
        ...
        result = lfw.LocalFlowWalkResult(
            positions=pos,
            velocities=vel,
            ordered_indices=true_ordering,
        )

        # Train autoencoder
        key, model_key = jax.random.split(key)
        autoencoder = lfw.nn.Autoencoder(rngs=model_key, n_dims=2)
