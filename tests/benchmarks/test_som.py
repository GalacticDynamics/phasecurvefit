...
def _defaults_metric(pos, vel):
    """Resolve the metric the way `SOMOrderer.order` does, standalone.

    ``metric``/``metric_scale`` ship as ``None`` -- "unset, follow ``init``" --
    so they cannot be handed to the core as-is. Resolving with ``init=None``
    gives what a standalone `order` call would use.
    """
    return DEFAULTS._resolve_metric(pos, vel, None)
...
class TestSOMCoreBenchmarks:
    ...
    def test_fit(self, benchmark, n, k):
        ...
        pos, vel = _helix(n)
        metric, metric_scale = _defaults_metric(pos, vel)
