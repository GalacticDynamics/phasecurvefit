...
using the **Nearest Neighbors with Momentum (NN+p)** algorithm from Nibauer et
al. (2022), plus tools for gap-filling via autoencoders.
...
- `AlignedMomentumDistanceMetric`: Original NN+p metric with velocity alignment
...
- `BruteForceStrategy()`: Default, computes distances to all points
- `KDTreeStrategy(k=...)`: KD-tree prefiltering (requires `jaxkd`)
...
- `train_autoencoder(autoencoder, result, config)`: Train on NN+p output
...
- `/src/localflowwalk/_src/`: Private implementation
  ...
  - `utils.py`: Utilities including `bounded_while_loop`
...
The algorithm uses a custom `bounded_while_loop` (in `_src/utils.py`)
implemented via `lax.scan` for reverse-mode autodiff compatibility. This avoids
`lax.while_loop` which doesn't support gradients.

### Result Structure

`LocalFlowWalkResult` contains:

- `ordered_indices`: Array of indices in walk order (-1 for skipped)
